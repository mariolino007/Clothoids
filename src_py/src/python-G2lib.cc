...
// Rimuovi la macro EXPORT_SYMBOL e usa direttamente PYBIND11_MODULE
PYBIND11_MODULE(G2lib, m) {
    ...
    // Classe per risoluzione sistema lineare 2x2
    py::class_<Solve2x2>(m, "Solve2x2")
        .def(py::init())
        .def("factorize", 
            [](Solve2x2* self, 
               std::tuple<std::tuple<real_type, real_type>, 
                          std::tuple<real_type, real_type>> A) {
                real_type A_[2][2];
        ...
        .def("solve", 
            [](Solve2x2* self, std::tuple<real_type, real_type> b) {
                real_type b_[2], x_[2];
                b_[0] = std::get<0>(b);
                b_[1] = std::get<1>(b);
                self->solve(b_, x_);
...
// Export esplicito SOLO per MSVC usando la sintassi corretta
#ifdef _MSC_VER
extern "C" __declspec(dllexport) PyObject* PyInit_G2lib(void) {
    return PyModule_Create(&PyModuleDef_G2lib);
}
#endif
